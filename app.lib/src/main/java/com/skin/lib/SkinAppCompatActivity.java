...
import com.skin.lib.com.skin.lib.views.Skinnable;

/**
 * Created by cheyanxu on 16/6/21.
 */
public class SkinAppCompatActivity extends AppCompatActivity implements LayoutInflaterFactory {
    private SkinnableViewInflater skinnableViewInflater;
    private SkinnableCallback skinnableCallback;
    ...
    public View onCreateView(View parent, String name, Context context, AttributeSet attrs) {
        if (skinnableViewInflater == null) {
            skinnableViewInflater = new SkinnableViewInflater();
        }

        final boolean isPre21 = Build.VERSION.SDK_INT < 21;

        final boolean inheritContext = isPre21 && shoundInheritContext((ViewParent) parent);


        return skinnableViewInflater.createView(parent, name, context, attrs, inheritContext,
    ...
    private boolean shoundInheritContext(ViewParent parent) {
        if (parent == null)
            return false;
    ...
    public void setDayNightMode(@AppCompatDelegate.NightMode int nightMode) {
        ...
        if (skinnableCallback != null) {
            skinnableCallback.beforeApplyDayNight();
        }
        ...
        if (skinnableCallback != null) {
            skinnableCallback.onApplyDayNight();
        }
    ...
    public void setSkinnableCallback(SkinnableCallback skinnableCallback) {
        this.skinnableCallback = skinnableCallback;
    }
